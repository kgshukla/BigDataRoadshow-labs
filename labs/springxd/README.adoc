...
- http://projects.spring.io/spring-xd/[Spring XD] v1.2.1+ installed 
- http://pivotal.io/big-data/pivotal-gemfire[GemFire] v8.1+ installed
- http://pivotal.io/big-data/pivotal-hd[PHD+Hawq] v3.0+  installed

=== Starting Pivotal Hadoop via Ambari
You may want to skip this section as your instructor have already started Pivotal hadoop and other services for you. 

If you want to learn how to do it your self, follow the instructions on this http://phd-start.cfapps.io/start_phd.pdf[website] to start Pivotal Hadoop.

Ask instructor for EC2 IPADDRESS.

=== Starting Spring XD

Login to your assigned machine using the following (terminal for mac/linux and putty for windows).

login/passwd is gpadmin/changeme

[source,bash]
----
$ssh gpadmin@<IP-address>
----
...
- Start the GemFire locator in a new shell terminal as gpadmin user
...
link:http://<IP-ADDRESS>:7676/gemfire-api/v1/ValuesFromXD[http://<IP-ADDRESS>:7676/gemfire-api/v1/ValuesFromXD] (It may not work out of the box for Internet explorer to show json output and start downloading some plugin. It works fine on Firefox or Chrome)
